...
const MyListings = () => {
    ...
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8'>
        <div>
          ...
          <p className='text-gray-600 mt-1'>Manage your social account listings</p>
        </div>
        <button onClick={()=>navigate('/create-listing')} className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded font-medium flex items-center space-x-2 m-4 md:mt-4 md:mt-0'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
          <StateCard title='Total Listing' value={userListings.length} icon={<Eye className='size-6 text-indigo-600' />}color='indigo' />

          <StateCard title='Active Listing' value={activeListings} icon={<CheckCircle className='size-6 text-green-600' />} color='green'/>

          <StateCard title='Sold' value={soldListings} icon={<TrendingUp className='size-6 text-indigo-600' />} color='indigo'/>

          <StateCard title='Total Listing' value={`${currency}${totalValue.toLocaleString()}`} icon={<DollarSign className='size-6 text-yellow-600' />} color='yellow'/>
      </div>
      ...
      (
        <div className='bg-white rounded-lg border border-gray-200 p-16 text-center'>
          <div className='w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4'>
            ...
            className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2'>Create First Listing </button>
        </div>
      ) : (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>{userListings.map((listing)=>(
          <div key={listing.id}
          className='bg-white rounded-lg border border-gray-200 hover:shadow-lg shadow-gray-200/70 trasition-shadow'>
            <div className='p-6'>
                  ...
                  <div className='flex justify-between items-start'>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <div className='bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3'>
                        ...
                        <button onClick={()=> setShowCredentialSubmission(listing)} className='text-nowrap'>
                          Status:{" "}
                          <span className={
                            listing.isCredentialSubmitted
              ...
              <div className='space-y-4'>
                <div className='grid grid-cols-2 gap-2 text-sm'>
                  ...
                  <span className={`flex items-center justify-end gap-1 text-green-800 ${getStatusColor(listing.status)}`}>
  ...
  {showCredentialSubmission && (
    <withdrawnModal onClose={()=> setShowWithdrawal(null)}/>
  )}
      {/* Footer */}
