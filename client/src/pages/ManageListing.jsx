...
import { addListing } from "../app/features/listingSlice";
...
const ManageListing = () => {
  ...
  const platform=['youtube', 'instagram', 'tiktok', 'facebook', 'linkedin', 'pinterest', 'snapchat', 'twitch', 'discord'];
  ...
  const ageRanges= ['13-17 years', '18-24 years', '25-34 years', '35-44 yaers', '45-54 years', '55+ years', 'Mixed ages'];
  ...
  // Get listing data for edit id 'id' is provided(edit mode)
  useEffect(()=>{
    if(!id) return;
 ...
 const handleSubmit = (e) => {
  ...
  return (
    <div className='min-h-screen py-8'>
      <div className='max-w-4xl max-auto px-4 sm:px-6 lg:px-8'>
        <div className='mb-8'>
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title='Basic Information'>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              <InputField label='Listing Title *' value={formData.title} placeholder='e.g., Preminum Travel Instagram Account' onChane={(v)=> handleInputChange('title', v)} required={true}/>
              ...
              <InputField label='Username/Handle *' value={formData.username} placeholder='@username' onChane={(v)=> handleInputChange('username', v)} required={true}/>
          ...
          <Section title='Account Metrics'>
            ...
            <InputField label='Followers Count *' type='number' min={0} value={formData.followers_count} placeholder='10000' onChane={(v)=> handleInputChange('followers_count', v)} required={true}/>

            <InputField label='Engagement_Rate (%)' type='number' min={0} max={100} value={formData.engagement_rate} placeholder='4' onChane={(v)=> handleInputChange('engagement_rate', v)}/>

            <InputField label='Monthly Views/Impressions' type='number' min={0} value={formData.monthly_views} placeholder='100000' onChane={(v)=> handleInputChange('monthly_views', v)}/>
            </div>

            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
              <InputField label='Primary Audience Country' value={formData.country} placeholder='United States' onChane={(v)=> handleInputChange('country', v)}/>
          ...
          <Section title='Pricing & Description'>
            <InputField label='Asking Price (USD) *' type='number' min={0} value={formData.price} placeholder='2500.00' onChane={(v)=>handleInputChange('price', v)} required={true}/>
          ...
          <Section title='Screenshots & proof'>
            ...
            {formData.images.length> 0 &&(
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mt-4'>{formData.images.map((img, index)=>(
...
const InputField=({label, value, onChane, placeholder, type='text', required=false, min=null, max=null})=>(
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <input type={type} min={min} max={max} placeholder={placeholder} value={value} onChange={(e)=>onChane(e.target.value)}
