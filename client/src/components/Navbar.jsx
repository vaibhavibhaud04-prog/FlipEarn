...
import { BoxIcon, Grid, GripIcon, ListIcon, MenuIcon, MessageCircle, MessageCircleMoreIcon, XIcon } from 'lucide-react'
  ...
  return (
   <nav className='h-20'>
                <div className='fixed left-0 top-0 right-0 z-100 flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white transition-all'>
                    <img onClick={()=>{navigate('/'); scrollTo}} src={assets.logo} alt="logo" className='h-10 cursor-pointer' />
                    ...
                    <div>
                        ...
                        <UserButton>
                            ...
                            <UserButton.MenuItems>
                                <UserButton.Action label='Message' labelIcon={<MessageCircleMoreIcon size={16}/>} onClick={()=> navigate('/message')}/>
                            </UserButton.MenuItems>
                ...
                <div className={`sm:hidden fixed inset-0 ${menuOpen ? 'w-full' : 'w-0'} overflow-hidden bg-white backdrop-blur shadow-xl rounded-lg z-[200] text-sm transition-all`}>
                    <div className='flex flex-col items-center justify-center h-full text-xl font-semibold gap-6 p-4'>
                        ...
                        <XIcon onClick={() => setMenuOpen(false)} className='absolute size-8 right-6 top-6 text-grey-500 hover:text-grey-700 cursor-pointer'/>
                    </div>
                </div>
            </nav>
  )
